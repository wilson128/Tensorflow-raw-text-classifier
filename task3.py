...
hiddenState = tf.tanh(tf.matmul(inputX,W) +b)
...
outputs = tf.matmul(hiddenState, W2) + b2
...
with tf.Session() as sess:
    ...
    for epoch in range(iter):
        x2, y2 = generateDataFeedForward(maxSeqLen,testData)
        _totalLoss, _predictions = sess.run([totalLoss,predictions],
                                            feed_dict = {inputX: x2, inputY: y2})
        numCorrect = 0
