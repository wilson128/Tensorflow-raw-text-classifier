...
def addToData (maxSeqLen, trainData, testData, fileName, classNum, linesToUse):
    #
    ...
    for whichLine in myInts.flat:
        ...
        if len(testData) - testLen < 1000:
            testData[len(testData)] = (classNum, temp)
        else:
    ...
    return (maxSeqLen, trainData, testData)
...
(maxSeqLen, trainData, testData) = addToData(maxSeqLen, trainData, testData, "Holmes.txt", 0, 10000)
(maxSeqLen, trainData, testData) = addToData(maxSeqLen, trainData, testData, "war.txt", 1, 10000)
(maxSeqLen, trainData, testData) = addToData(maxSeqLen, trainData, testData, "william.txt", 2, 10000)
...
filters = tf.Variable(np.random.normal(0, 0.05, (2560, 8)), dtype=tf.float32)
sequenceOfLetters = tf.unstack(inputX, axis=2)

seq10 = []
for i in range(maxSeqLen - 10):
    state = sequenceOfLetters[i]
    for j in range(9):
        state = tf.concat(
            [state, sequenceOfLetters[i + j]], 1)
    seq10.append(state)

currentState = initialState

special = tf.Variable(np.random.normal(0, 0.05, (batchSize, hiddenUnits)), dtype=tf.float32)

states = [special for i in range(10)]

# now we implement the forward pass
for time in seq10:
    res = tf.matmul(time, filters)
    # concatenate the state with the input, then compute the next state
    state = tf.concat([res, currentState], 1)
    state = tf.concat([state, states.pop(0)], 1)
    nextState = tf.tanh(tf.matmul(state, W) + b)
...
with tf.Session() as sess:
    ...
    for epoch in range(iter):
        x2 = np.stack(testData[j][1] for j in range(epoch * 100, (epoch * 100) + 100))
        y2 = np.stack(np.array((testData[j][0])) for j in range(epoch * 100, (epoch * 100) + 100))
        _currentState_test = np.zeros((batchSize, hiddenUnits))
        _predictions, _totalLoss, _currentState_test = sess.run(
            [predictions, totalLoss, currentState],
            feed_dict={
                inputX: x2,
                inputY: y2,
                initialState: _currentState_test
            })
        numCorrect = 0
